...
export const makeSocket = (config) => {
    ...
    let qrTimer;
    let closed = false;
    // O servidor só aceita o link_code_companion_reg depois de responder o
    // pair-device. Pedir o código antes disso (ex.: logo após criar o socket)
    // devolvia um código que o celular rejeitava. Estas flags fazem a chamada
    // esperar a prontidão em vez de disparar cedo.
    ...
    const end = async (error) => {
        ...
        // Uma espera de pareamento pendente precisa ser rejeitada aqui: senão a
        // Promise fica pendurada para sempre quando a conexão cai antes do
        // pair-device chegar.
        pairingReady = false;
    ...
    const sendPairingCodeRequest = async (phoneNumber, customPairingCode) => {
        ...
        // O WhatsApp valida o `companion_platform_display` do link-code e só
        // aceita pares canônicos (ex.: "Chrome (Windows)"). Com o browser UWP o
        // rótulo saía "UWP (Windows)" e o código gerado era rejeitado no
        // celular. O display é normalizado; o `companion_platform_id` continua
        // vindo do browser real para a stack de voz seguir como UWP.
        ...
        try {
            ...
            // Sem limpar aqui, um código morto ficaria salvo no auth state e
            // seria reusado numa nova tentativa.
    ...
    ws.on('CB:iq,type:set,pair-device', async (stanza) => {
        ...
        await sendNode(iq);
        // A partir daqui o servidor aceita o link_code_companion_reg; libera
        // qualquer pedido de código que tenha chegado antes.
        pairingReady = true;
        pendingPairingResolve?.();
