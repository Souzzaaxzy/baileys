...
export const makeChatsSocket = (config) => {
    ...
    const getBusinessProfile = async (jid) => {
        ...
        // The BinaryNode walk lives in `Utils/business.js` so the same parser is
        // shared with the business socket. It returns the original fields plus
        // `categories` (all of them, not only the first) and `coverPhotoId`, which
        // are additive: every key the previous inline parser produced is still
        // present with the same value.
        return parseBusinessProfileNode(results);
    };
