...
import { logSuppressedRetry } from '../Utils/sender-key-rotation.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const { userDevicesCache, devicesMutex, ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, suppressedRetryRegistry, registerSocketEndHandler, issuePrivacyTokens, fetchAccountReachoutTimelock, placeholderResendCache, getUSyncDevices, createParticipantNodes } = sock;
    const getLIDForPN = signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping);
    /** this mutex ensures that each retryRequest will wait for the previous one to finish */
    const retryMutex = makeMutex();
    // EXPERIMENTAL: index of Sender Key Distribution Messages seen on receive.
    // Used to corroborate a rotation when a group message does not decrypt —
    // a late joiner/lost key does NOT bring an SKDM for a group it is already
    // in, a rotation does. Structure only; the report carries no key material.
    const skdmIndex = createSkdmIndex();
    ...
    /**
     * Sends a `<preaccept>` to the caller, signalling that the offer arrived and
     * this device is ringing — the SIP 180 Ringing equivalent.
     *
     * This is NOT an acceptance: it does not commit to the call, does not
     * negotiate media, and the library cannot carry call audio/video at all.
     * Use it to make the caller's UI show an accurate ringing state (and, if the
     * caller chooses, to start transport warm-up).
     *
     * Wire shape follows the wacrg working model
     * (docs/spec/stanzas/call-preaccept.md), which is marked `probable` and not
     * yet confirmed against a real capture — hence no `reason`/`count` attrs
     * here, unlike `<reject>`.
     *
     * @param callId   logical call session id, copied from the offer
     * @param callFrom JID of the call creator (the caller)
     */
    const preacceptCall = async (callId, callFrom) => {
        const stanza = {
            tag: 'call',
            attrs: {
                from: authState.creds.me.id,
    ...
    /**
     * Sends a call-control stanza and requires the server ack.
     *
     * `query` resolves with `undefined` when the ack never arrives (the socket
     * layer turns a timeout into `undefined` rather than throwing). For call
     * signaling that silence is a FAILURE, not success: reporting a placed call
     * that the server never acknowledged is how a caller ends up with a success
     * message and no ringing anywhere. So this wrapper turns a missing ack into
     * a thrown error.
     */
    const sendCallStanza = async (node, timeoutMs) => {
        const ack = await query(node, timeoutMs);
        if (!ack || typeof ack !== 'object' || !ack.tag) {
            throw new Boom('call stanza was not acknowledged by the server', { statusCode: 408 });
        }
        return ack;
    };
    /**
     * Places a **1:1** voice/video call: builds the `<call><offer>` and sends it
     * with `query`, so the returned promise resolves with the server ack.
     *
     * ## Signaling only
     *
     * This makes the call EXIST — the callee's device rings and the call can be
     * answered, negotiated and hung up. It does **not** carry audio: call media
     * runs over SRTP/UDP with keys negotiated outside this library, which is a
     * separate stack this library does not implement. Use this for call control
     * (place, ring, answer, end), not for streaming sound.
     *
     * The call key is encrypted per recipient device with the existing Signal
     * sessions (`createParticipantNodes`), exactly like a multi-device message,
     * and delivered as `<enc>` nodes inside `<destination>`.
     *
     * @param toJid  callee account JID (or LID)
     * @param options.isVideo  advertise video instead of audio only
     * @param options.callId   reuse a call id instead of generating one
     * @param options.timeoutMs ack timeout, defaults to the socket query timeout
     * @returns `{ id, to, stanzaId }`
     */
    const offerCall = async (toJid, options = {}) => {
        if (typeof toJid !== 'string' || !toJid) {
            throw new Boom('offerCall: a callee JID is required', { statusCode: 400 });
        }
        if (typeof options === 'boolean') {
            // Backwards-compatible call shape: offerCall(jid, isVideo)
            options = { isVideo: options };
        }
        const { isVideo = false, callId = generateCallId(), timeoutMs } = options;

        const meId = authState.creds.me.id;
        const devices = await getUSyncDevices([toJid], true, false);
        const deviceJids = devices.map(({ user, device }) => jidEncode(user, 's.whatsapp.net', device));
        await assertSessions(deviceJids, true);

        // A random 32-byte call key, wrapped for each device through Signal.
        ...
        const node = buildOffer({
            ...
            stanzaId,
            video: isVideo,
            deviceIdentity: shouldIncludeDeviceIdentity
                ? encodeSignedDeviceIdentity(authState.creds.account, true)
                : undefined
        });

        await sendCallStanza(node, timeoutMs);
        return { id: callId, to: toJid, stanzaId };
    };
    /**
     * Places a **group** call: builds the group-bound `<call><offer>` and sends
     * it with `query`.
     *
     * The differences from `offerCall` are the ones that matter for groups: the
     * stanza is addressed to the call object (`<call-id>@call`), carries
     * `group-jid`, and the participants travel as a `<group_info>` roster instead
     * of per-device `<enc>` nodes. The group's media key is distributed later,
     * per epoch, so no call key is sent here.
     *
     * Signaling only, same as `offerCall`: the call exists and can be joined by
     * group members; no audio is carried.
     *
     * @param groupJid   the group the call is bound to
     * @param jids       members to invite (the caller is added automatically)
     * @param options.isVideo   advertise video
     * @param options.callId    reuse a call id
     * @param options.timeoutMs ack timeout
     * @returns `{ id, groupJid, stanzaId, participants }`
     */
    const offerGroupCall = async (groupJid, jids = [], options = {}) => {
        if (typeof groupJid !== 'string' || !groupJid.endsWith('@g.us')) {
            throw new Boom('offerGroupCall: a group JID is required', { statusCode: 400 });
        }
        const { isVideo = false, callId = generateCallId(), timeoutMs } = options;

        const meId = authState.creds.me.id;
        const meLid = authState.creds.me.lid || meId;
        const targets = [...new Set(jids.filter(Boolean))];
        const devices = targets.length ? await getUSyncDevices(targets, true, false) : [];
        const roster = buildRoster(targets, devices, meLid);
        ...
        await sendCallStanza(node, timeoutMs);
        return { id: callId, groupJid, stanzaId, participants: roster.length };
    };
    /**
     * Ends a call this client started (or is part of).
     *
     * @param callId       logical call id from `offerCall`/`offerGroupCall`
     * @param options.to   peer to address; defaults to the call object
     * @param options.reason optional terminate reason (e.g. `timeout`, `busy`)
     */
    const terminateCall = async (callId, options = {}) => {
        if (!callId) {
            throw new Boom('terminateCall: a call id is required', { statusCode: 400 });
        }
        const meId = authState.creds.me.id;
        const meLid = authState.creds.me.lid || meId;
        const node = buildTerminate({
            callId,
            callCreator: meLid,
            to: options.to,
        ...
        await sendCallStanza(node, options.timeoutMs);
        return { id: callId };
    };
            ...
            await Promise.all([
                receiptMutex.mutex(async () => {
                    ...
                    if (attrs.type === 'retry') {
                        ...
                        // EXPERIMENTAL: a rotated message is withheld from retries.
                        // Answering one would re-send the content pairwise to a
                        // participant the rotation deliberately left without the
                        // key, undoing the isolation. The message stays
                        // undecryptable for that participant, which is the point.
                        if (suppressedRetryRegistry?.isSuppressed(ids[0])) {
                            logSuppressedRetry(logger, {
                                messageId: ids[0],
                                participant: key.participant,
                                groupJid: remoteJid
                            });
                        }
                        else if (ids[0] && key.participant && (await willSendMessageAgain(ids[0], key.participant))) {
                            if (key.fromMe) {
                                try {
    ...
    const handleMessage = async (node) => {
        ...
        let acked = false;
        try {
            const { fullMessage: msg, category, author, decrypt } = decryptMessageNode(node, authState.creds.me.id, authState.creds.me.lid || '', signalRepository, logger, skdmIndex);
            ...
            // EXPERIMENTAL: a detected selective-distribution message is emitted
            // HERE, immediately after decryption and BEFORE the CIPHERTEXT/retry
            // block below.
            //
            // Why not at the end of the handler: the whole handler runs under one
            // global `messageMutex`, and the retry block inside it awaits
            // `retryRequestDelayMs` (5s in Lizzy). Anything emitted after that
            // block only reaches the application ~5s later, which is precisely
            // why the anti fired late — or only on a big enough burst — and why
            // the bot felt stuck. Measured before this fix: the upsert arrived at
            // 5011ms.
            let detectedSelectiveDistribution = false;
            await messageMutex.mutex(async () => {
                await decrypt();
                if (msg.key?.remoteJid && msg.key?.id && msg.message && messageRetryManager) {
                    messageRetryManager.addRecentMessage(msg.key.remoteJid, msg.key.id, msg.message);
                }
                if (msg.selectiveDistribution) {
                    detectedSelectiveDistribution = true;
                ...
                if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
                    if (msg.selectiveDistribution) {
                        // EXPERIMENTAL: a message flagged as selective distribution gets
                        // NO retry request.
                        //
                        // Two reasons. Correctness: a retry asks the sender to resend,
                        // and the sender is the one withholding the key — retrying
                        // hands them exactly the fallback they want. Speed: the retry
                        // path awaits `retryRequestDelayMs` (5s in Lizzy) under the
                        // global `messageMutex`, and `upsertMessage` is a BUFFERED
                        // event that only flushes when the handler returns — so the
                        // detection could not reach the application until the retry
                        // finished. Measured before this fix: 5011ms. Skipping the
                        // retry removes both problems.
                        acked = true;
                        await sendMessageAck(node, NACK_REASONS.UnhandledError);
                    }
                    else if (msg?.messageStubParameters?.[0] === MISSING_KEYS_ERROR_TEXT) {
                        acked = true;
                        return sendMessageAck(node, NACK_REASONS.ParsingError);
                    }
                    else if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
                        ...
                        await retryMutex.mutex(async () => {
                            try {
                                ...
                                // NOTE: the pacing delay is applied OUTSIDE the mutex
                                // (below). With it inside, every undecryptable message
                                // held the global retry mutex for `retryRequestDelayMs`,
                                // so a burst serialized behind it — the next message
                                // waited seconds before it could even be processed,
                                // which delayed detection and stalled the handler.
                            }
                            catch (err) {
                        ...
                        // Pacing between retry requests, without blocking the queue.
                ...
                cleanMessage(msg, authState.creds.me.id, authState.creds.me.lid);
                // A detected selective-distribution message was already emitted right
                // after decryption (above), so the anti sees it immediately. Emitting
                // again here would duplicate it and the anti would punish twice.
                if (!detectedSelectiveDistribution) {
                    await upsertMessage(msg, node.attrs.offline ? 'append' : 'notify');
                }
            });
        }
