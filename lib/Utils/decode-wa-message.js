...
import { buildSelectiveDistributionReport, isSelectiveDistributionFailure } from './selective-distribution-detector.js';
...
export const decryptMessageNode = (stanza, meId, meLid, repository, logger, skdmIndex = null) => {
        ...
        async decrypt() {
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    ...
                    // EXPERIMENTAL: a GROUP stanza carrying a PAIRWISE enc.
                    //
                    // A group message normally rides a Sender Key (`skmsg`). A
                    // pairwise `enc type="msg"` on a group stanza is the shape of
                    // a retry response — the sender re-encrypting the message for
                    // one device — which is also the transport used to hand
                    // content to a device that was deliberately excluded from the
                    // rotated key.
                    //
                    // Reported as a flag only: a retry ALSO happens for benign
                    // reasons (a device genuinely failed to decrypt), so this is
                    // corroborating structure, never proof on its own. The
                    // decision belongs to the caller.
                    if (tag === 'enc' && (attrs.type === 'msg' || attrs.type === 'pkmsg') && isJidGroup(stanza.attrs.from)) {
                        fullMessage.pairwiseGroupPayload = true;
                    }
                    ...
                    try {
                        ...
                        if (msg.senderKeyDistributionMessage) {
                            // A fresh SKDM for a group this device is already in is
                            // the delivery step of a rotation. Record it so that a
                            // following undecryptable message from the same author
                            // can be corroborated as a rotation instead of being
                            // mistaken for "joined late / lost the key".
                            skdmIndex?.record?.(sender, author);
                        ...
                        // EXPERIMENTAL: flag a group message that was delivered but
                        // deliberately made unreadable for this device (rotated
                        // Sender Key distributed to a subset, `decrypt-fail="hide"`).
                        // Reported as structure only; the caller decides what to do.
                        if (isSelectiveDistributionFailure({ encType: attrs.type, encAttrs: attrs, error: err })) {
                            const report = buildSelectiveDistributionReport({
                                stanza,
                                error: err,
                                skdmRecentMs: skdmIndex?.ageOf?.(sender, author) ?? null
                            });
                            fullMessage.selectiveDistribution = report;
                            logger.warn(report, 'group message with selective distribution detected');
                        }
                    }
                }
            }
