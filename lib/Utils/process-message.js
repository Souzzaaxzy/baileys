    ...
    if (protocolMsg) {
        ...
        switch (protocolMsg.type) {
            ...
            case proto.Message.ProtocolMessage.Type.MARK_AS_VERIFIED_ACTION:
                /*
                 * Lia@Changes --- EXPERIMENTAL: MarkAsVerifiedAction (field 32).
                 *
                 * WHAT THE EVIDENCE SAYS (not what the name suggests):
                 *   - the action arrives as a ProtocolMessage of type 36;
                 *   - the reference client implementation distributes it as a
                 *     CHAT-level update (`chats.update`), carrying the action
                 *     object itself — not a user badge;
                 *   - it is emitted, never asserted. This handler therefore only
                 *     OBSERVES and REPORTS; it does not claim any effect.
                 *
                 * Nothing here decides whether the account gains any verified
                 * state. It surfaces the payload so callers/tests can inspect
                 * what actually arrived.
                 */
                if (protocolMsg.markAsVerifiedAction) {
                    const markAsVerifiedAction = protocolMsg.markAsVerifiedAction;
                    logger?.info(
                        {
                            // Structure only: the identity key is reduced to its
                            // LENGTH. Never log the key itself.
                            chatId: chat.id,
                            userJidString: markAsVerifiedAction.userJidString,
                            verified: markAsVerifiedAction.verified,
                            actionSeq: markAsVerifiedAction.actionSeq != null
                                ? String(markAsVerifiedAction.actionSeq)
                                : null,
                            verifiedIdentityKeyBytes: markAsVerifiedAction.verifiedIdentityKey
                                ? markAsVerifiedAction.verifiedIdentityKey.length
                                : 0
                        },
                        'received markAsVerifiedAction'
                    );
            ...
            case proto.Message.ProtocolMessage.Type.CHAT_THEME_SETTING:
                /*
                 * Lia@Changes --- EXPERIMENTAL: ChatThemeSetting (field 30, type 34).
                 *
                 * OBSERVATIONAL ONLY. This reports a theme change that ARRIVED;
                 * it does not apply anything and does not claim any visual
                 * effect. Whether the official client honors an inbound theme is
                 * not established here — this handler exists so the laboratory
                 * can compare a SENT payload with a RECEIVED one.
                 *
                 * `customImage` carries `mediaKey` / `file*Sha256`: reported as
                 * LENGTH only, never as content.
                 */
                if (protocolMsg.chatThemeSetting) {
                    const theme = protocolMsg.chatThemeSetting;
                    const variantes = ['defaultWallpaper', 'solidColor', 'stockImage', 'customImage', 'animatedWallpaper'];
                    const variante = variantes.find(v => theme[v] !== undefined && theme[v] !== null) || null;
                    const wall = variante ? theme[variante] : null;
                    logger?.info(
                        {
                            chatId: chat.id,
                            settingTimestampMs: theme.settingTimestampMs != null ? String(theme.settingTimestampMs) : null,
                            clearTheme: theme.clearTheme,
                            colorSchemeId: theme.colorSchemeId,
                            wallpaper: variante,
                            wallpaperId: wall?.stockImageId ?? wall?.animatedWallpaperId ?? null,
                            dimLevel: wall?.dimLevel ?? null,
                            mediaKeyBytes: wall?.mediaKey ? wall.mediaKey.length : 0
                        },
                        'received chatThemeSetting'
                    );
                    ev.emit('chats.update', [
                        {
                            id: chat.id,
                            chatThemeSetting: theme,
                            chatThemeVariant: variante
                        }
                    ]);
                }
                break;
