...
import { toProductListInfo } from './business.js';
...
/**
 * Build a `productMessage` from the friendly `product` input.
 *
 * Handles both commerce shapes the protocol supports:
 *
 *  - SPM (single product): `message.product` -> `ProductSnapshot`, with the
 *    product image uploaded through `prepareWAMessageMedia`.
 *  - Catalog card: `message.catalog` -> `CatalogSnapshot` (`catalogImage`,
 *    `title`, `description`). The catalog was already a field on
 *    `proto.Message.ProductMessage` but this builder ignored it, so a catalog
 *    card could not be produced. It is now built the same way the product is:
 *    the image goes through the normal media upload.
 *
 * A message may carry `product`, `catalog`, or both.
 */
const prepareProductMessage = async (message, options) => {
    ...
    const { image, catalog, ...content } = message;
    if (catalog) {
        // The catalog card's picture is what the user sees as the "shop" image.
        const { catalogImage: catalogImageInput, image: catalogImageAlias, ...catalogRest } = catalog;
        const catalogImageSource = catalogImageInput || catalogImageAlias;
        const { imageMessage: catalogImage } = await prepareWAMessageMedia({ image: catalogImageSource }, options);
        content.catalog = {
            ...catalogRest,
            catalogImage
        };
    ...
    if (message.product) {
        const { imageMessage } = await prepareWAMessageMedia({ image: image || message.product.productImage }, options);
        content.product = {
            currencyCode: 'IDR',
...
const prepareStickerPackMessage = async (message, options) => {
    const { cover, stickers = [], name = '📦 Sticker Pack', publisher = 'GitHub: Souzzaaxzy', description = '🏷️ Souzzaaxzy/baileys' } = message;
...
const prepareNativeFlowButtons = (message) => {
    ...
    // Lia@Changes --- Tolerate a card/message without `nativeFlow`.
    //
    // Before this, a carousel card without `nativeFlow` (e.g. a plain video
    // card) reached `buttons.buttons` with `buttons === undefined` and threw
    // "Cannot read properties of undefined (reading 'buttons')", killing the
    // whole carousel. A missing field is simply no buttons.
...
/**
 * Lia@Changes --- Resolve the media header of a carousel card, honouring `ptv`.
 *
 * A card whose video is sent as `ptv: true` (video note) is prepared by
 * `prepareWAMessageMedia` into a `ptvMessage`, not a `videoMessage`. The plain
 * `hasValidCarouselHeader` only knew `imageMessage`/`videoMessage`/
 * `productMessage`, so a `ptv` video card was rejected with "Invalid media type
 * for carousel card" even though the upload had already succeeded. Map the
 * `ptvMessage` back to the header slot the proto expects (`videoMessage`),
 * which also keeps `hasValidCarouselHeader` meaningful.
 */
export const resolveCarouselHeader = (prepared) => {
    const header = prepared && typeof prepared === 'object' ? { ...prepared } : {};
    if (header.ptvMessage) {
        header.videoMessage = header.ptvMessage;
        delete header.ptvMessage;
    }
    return header;
};
/**
 * Lia@Changes --- Experimental: builds a poll whose options carry images.
 *
 * The wire format is a parent poll (`pollCreationMessageV3`) marked with
 * `pollContentType: IMAGE`, followed by one `imageMessage` per option, each
 * associated to the poll via `messageContextInfo.messageAssociation`
 * (`MEDIA_POLL`). The poll options themselves only carry `optionName` and an
 * `optionHash` that binds the option to its image.
 *
 * `optionHash` is the SHA-256 of `sha256(optionName)` concatenated with the
 * base64 `fileSha256` of the uploaded image -- see
 * `generatePollOptionHash` below.
 *
 * Images are uploaded through the existing `prepareWAMessageMedia`, so the
 * regular media pipeline (encryption, thumbnail, upload, cache) is reused.
 * The prepared `imageMessage`s are written to `options.imagePollImages` so the
 * caller (`sendMessage`) can relay them as associated children without
 * uploading anything twice.
 *
 * @returns the poll content (`pollCreationMessageV3` + `messageContextInfo`).
 */
const prepareImagePollMessage = async (poll, options) => {
    if (!Array.isArray(poll.options) || poll.options.length < 2) {
        throw new Boom('An image poll needs at least 2 options', { statusCode: 400 });
    }
    for (const option of poll.options) {
        if (!option || typeof option.name !== 'string' || !option.name.trim()) {
            throw new Boom('Every image poll option needs a name', { statusCode: 400 });
        }
        if (!hasNonNullishProperty(option, 'image')) {
            throw new Boom('Every image poll option needs an image', { statusCode: 400 });
        }
    }
    const selectableCount = poll.selectableCount ?? 1;
    if (selectableCount < 1 || selectableCount > poll.options.length) {
        throw new Boom(`poll.selectableCount should be >= 1 and <= ${poll.options.length}`, { statusCode: 400 });
    }
    // Upload each image through the regular media pipeline, in parallel, keeping
    // the original order (the option hash depends on the matching image).
    const preparedImages = await Promise.all(poll.options.map(option => prepareWAMessageMedia({ image: option.image, mimetype: option.mimetype }, options)));
    const pollOptions = poll.options.map((option, index) => {
        const image = preparedImages[index].imageMessage;
        if (!image?.fileSha256) {
            throw new Boom('Failed to prepare the option image', { statusCode: 400 });
        }
        return {
            optionName: option.name,
            optionHash: generatePollOptionHash(option.name, image.fileSha256)
        };
    });
    const pollCreationMessage = {
        ...
        endTime: poll.endDate ? poll.endDate.getTime() : undefined,
        hideParticipantName: poll.hideVoter ?? false,
        allowAddOption: poll.canAddOption ?? false
    };
    // Hand the children back to sendMessage so it can relay them associated to
    // this poll (see the MEDIA_POLL block in messages-send.js).
    if (options.imagePollHolder) {
        options.imagePollHolder.images = preparedImages;
    }
    return {
...
/**
 * Lia@Changes --- Experimental: `optionHash` for an image poll option.
 *
 * Matches the client: hex(sha256(hex(sha256(optionName)) + base64(fileSha256))).
 */
export const generatePollOptionHash = (optionName, fileSha256) => {
    const nameHashHex = sha256(optionName).toString('hex');
    const fileHashB64 = Buffer.from(fileSha256).toString('base64');
    return sha256(nameHashHex + fileHashB64).toString('hex');
};
export const generateWAMessageContent = async (message, options) => {
    ...
    // A productMessage may carry either an SPM (`product`) or a catalog card
    // (`catalog`); both are the same proto message, so the guard accepts either.
    else if (hasNonNullishProperty(message, 'product') || hasNonNullishProperty(message, 'catalog')) {
    ...
    // Lia@Changes --- Add experimental support for polls whose options carry images
    //
    // The poll itself stays a regular `pollCreationMessageV3`; what makes it a
    // "photo poll" is `pollContentType: IMAGE` plus an `optionHash` per option.
    // The images are NOT stored here: each one travels as its own `imageMessage`
    // associated with the poll through
    // `messageContextInfo.messageAssociation` (see messages-send.js).
    else if (hasNonNullishProperty(message, 'imagePoll')) {
        m = await prepareImagePollMessage(message.imagePoll, options);
    }
    ...
    // Multi-product (MPM) catalog card.
    //
    // On the wire WhatsApp has no `ProductListMessage` type: the MPM is a
    // `listMessage` with `listType = PRODUCT_LIST` carrying a `productListInfo`
    // (productSections + headerImage + businessOwnerJid). Accepting
    // `message.productList` builds exactly that, so a catalog browse card can be
    // sent without the caller knowing the quirk.
        ...
        const listMessage = {
            ...
            productListInfo: toProductListInfo(message.productList)
        };
        m = { listMessage };
    }
            ...
            carouselMessage: {
                cards: await Promise.all(message.cards.map(async (card) => {
                    ...
                    if (hasNonNullishProperty(card, 'product')) {
                        ...
                        carouselHeader = resolveCarouselHeader(await prepareWAMessageMedia(card, options).catch(() => ({})));
                    }
                    ...
                    if (hasOptionalProperty(card, 'text')) {
                        // Lia@Changes --- A `text` card may ALSO carry media.
                        //
                        // The `text` branch used to skip the header entirely, so
                        // a video card with `text` (instead of `caption`) silently
                        // dropped the video and produced an empty card. When the
                        // header is valid, attach it regardless of which body
                        // field was used.
                        ...
                        // Lia@Changes --- Always create the header when the media
                        // is valid. Before this, a carousel card with media but no
                        // `caption` never created `carouselCard.header`, and the
                        // `Object.assign` below threw "Cannot convert undefined or
                        // null to object", taking the whole carousel down. A media
                        // card is valid with or without a caption.
                        carouselCard.header = {
                            title: card.title || '',
                            subtitle: card.subtitle || '',
                            hasMediaAttachment: true
    ...
    // Lia@Changes --- Add "canBeReshared" boolean to mark the message as
    // reshareable (contextInfo.featureEligibilities.canBeReshared).
    //
    // Necessário porque status postado por biblioteca NÃO mostra o botão de
    // repostar no cliente, mesmo com "Allow Sharing" ligado nas configurações
    // de privacidade da conta: o cliente lê essa permissão do PRÓPRIO payload.
    // Descoberto em WhiskeySockets/Baileys#2633.
    if (hasOptionalProperty(message, 'canBeReshared') && !!message.canBeReshared) {
        const messageType = Object.keys(m)[0];
        const key = m[messageType];
        if (key) {
            key.contextInfo = {
                ...
                featureEligibilities: {
                    ...(key.contextInfo?.featureEligibilities || {}),
                    canBeReshared: true
                }
            };
        }
        delete message.canBeReshared;
    }
