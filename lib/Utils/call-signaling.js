...
/**
 * Normalise a device record / JID into the BARE account JID (no device).
 *
 * The captured roster uses the bare LID on `<user jid>` and the device-qualified
 * JID only on the nested `<device jid>`. Using the qualified form on `<user>`
 * produces a roster the server does not recognise as participants.
 */
const bareJid = (jid) => {
 ...
 *
 * Shape taken from the authoritative initial-group-call capture:
 *
 * ```xml
 * <user jid="156535032389744@lid">          <!-- BARE account jid -->
 *   <device jid="156535032389744:14@lid">   <!-- device-qualified -->
 *     <capability ver="1">0105f709e0bb13</capability>
 *   </device>
 * </user>
 * ```
 *
 * Only the LOCAL device carries a capability child; remote devices have none.
 * The local entry is built first and always present (it is the call creator).
 *
 * @param jids     accounts to include (bare or device-qualified)
 * @param devices  raw device records from `getUSyncDevices`
 * @param selfJid  the local device JID
 */
export const buildRoster = (jids, devices, selfJid) => {
    const selfUser = selfJid ? selfJid.split('@')[0].split(':')[0] : null;
    const byUser = new Map();

    const ensure = (accountJid) => {
        if (!byUser.has(accountJid)) byUser.set(accountJid, { jid: accountJid, devices: [] });
        return byUser.get(accountJid);
    };

    // The creator's own entry goes first, with its exact active device JID.
    if (selfJid) {
        const selfAccount = bareJid(selfJid) || selfJid;
        ensure(selfAccount).devices.push({ jid: selfJid, capability: CAPABILITY_OFFER });
    }

    for (const device of devices || []) {
        if (!device?.user) continue;
        const server = device.server || 's.whatsapp.net';
        const accountJid = `${device.user}@${server}`;
        const deviceJid = jidEncode(device.user, server, device.device);
        const entry = ensure(accountJid);
        if (entry.devices.some((d) => d.jid === deviceJid)) continue;
        entry.devices.push({
            jid: deviceJid,
    ...
    // Every requested account must be present even if discovery returned nothing,
    // so the roster reflects the selection instead of silently dropping people.
    const roster = [...byUser.values()];
    for (const jid of jids || []) {
        const account = bareJid(jid);
        if (!account) continue;
        if (!roster.some((entry) => entry.jid === account)) {
            roster.push({ jid: account, devices: [{ jid, capability: null }] });
        }
    }
    return roster;
};
