...
import { CallStatus } from '../Types/Call.js';
...
export const fetchLatestBaileysVersion = async (options = {}) => {
    const URL = 'https://raw.githubusercontent.com/Souzzaaxzy/baileys/main/lib/Defaults/index.js';
    try {
...
/**
 * Maps the child tag of a `<call>` stanza to a normalised call status.
 *
 * `offer_notice` collapses into `offer`, and any unmapped tag falls back to
 * `ringing`. `terminate` splits on `reason=timeout`, because a timeout is how an
 * unanswered call ends and is the condition that produces a missed-call stub.
 *
 * The status strings come from CallStatus so consumers have a single source of
 * truth (see lib/Types/Call.js). Behaviour is unchanged from the previous
 * switch — only the literals were replaced with the named constants.
 */
export const getCallStatusFromNode = ({ tag, attrs }) => {
    // Attrs can be missing on a malformed node — read defensively so a bad
    // stanza classifies instead of throwing out of the call handler.
    const { reason } = attrs || {};
    switch (tag) {
        case 'offer':
        case 'offer_notice':
        ...
        case 'terminate':
            ...
            return reason === 'timeout' ? CallStatus.Timeout : CallStatus.Terminate;
        case 'preaccept':
            return CallStatus.PreAccept;
        case 'transport':
            return CallStatus.Transport;
        case 'relaylatency':
            return CallStatus.RelayLatency;
        case 'reject':
