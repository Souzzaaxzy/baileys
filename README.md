...
- [👁️ Other Message Options](#%EF%B8%8F-other-message-options)
   ...
   - [🎨 ChatThemeSetting (experimental)](#-chatthemesetting-experimental)
...
#### 🎨 ChatThemeSetting (experimental)

> [!IMPORTANT]
> This is **experimental and observational**. Nothing here claims that a chat
> theme is applied, on this device or on anyone else's. The name of the field
> is not evidence.

The theme travels as a **`ProtocolMessage`**, not as a top-level message
property and not as App State:
...
message ChatThemeSetting {
  ...
  oneof wallpaper {
    ChatDefaultWallpaper     defaultWallpaper   = 10;
    ChatSolidColorWallpaper  solidColor         = 11;
    ChatStockImageWallpaper  stockImage         = 12;
    ChatCustomImageWallpaper customImage        = 13;
    ChatAnimatedWallpaper    animatedWallpaper  = 14;
  }
}
```

The nested wallpapers (verified against WhatsApp Web's own internal spec):

```proto
message ChatDefaultWallpaper     { optional bool   isDoodleEnabled = 1; }
message ChatSolidColorWallpaper  { optional string colorLight = 1; optional string colorDark = 2; optional bool isDoodleEnabled = 3; }
message ChatStockImageWallpaper  { optional string stockImageId = 1; optional float dimLevel = 2; }
message ChatCustomImageWallpaper { optional string directPath = 1; optional bytes mediaKey = 2; optional bytes fileEncSha256 = 3; optional bytes fileSha256 = 4; optional float dimLevel = 5; }
message ChatAnimatedWallpaper    { optional string animatedWallpaperId = 1; optional float dimLevel = 2; }
```

> [!NOTE]
> `ChatAnimatedWallpaper` (variant 14) was **missing from this fork's generated
> WAProto** — encoding it silently produced 0 bytes. `scripts/add-chat-animated-wallpaper.js`
> is a deterministic, idempotent delta that adds it. The other four variants
> were already present.

Sending (through the existing relay — no second transport):

```javascript
await sock.sendChatTheme({
   jid,                                  // @g.us or @s.whatsapp.net
   settingTimestampMs: Date.now(),
   colorSchemeId: 'Tonal',               // optional
   wallpaper: { stockImage: { stockImageId: 'wall-1', dimLevel: 0.5 } }
   // wallpaper: { animatedWallpaper: { animatedWallpaperId: 'wall-9', dimLevel: 0.3 } }
   // wallpaper: { solidColor: { colorLight: '#FFFFFFFF', colorDark: '#FF000000' } }
   // clearTheme: true
})
```
...
sock.ev.on('chats.update', updates => {
   for (const u of updates) {
      if (u.chatThemeSetting) { /* what actually arrived */ }
   }
})
```
...
- **The oneof is not enforced by the generated `encode`.** It uses
  `hasOwnProperty`, so setting two wallpaper members writes **both** to the
  wire and the receiver resolves by the last one. `buildChatThemeSetting`
  therefore enforces exclusivity itself and refuses more than one variant.
- **Direction is not established.** Public reporting describes chat themes as
  personal (each side picks their own). Sending is an experiment: the client
  may accept and ignore it.
- **`mediaKey` / `file*Sha256` are never logged**, only their byte length.
- **Fails closed.** An invalid JID, an unknown variant or a mixed oneof throws;
  nothing is sent.
